...
use termsfx_config::TermsfxConfig;
...
impl TermsfxConfigLoader {
    ...
    fn load_config(config_path: &PathBuf) -> TermsfxConfig {
        ...
        let config = settings.try_deserialize::<TermsfxConfig>().unwrap();

        config.validate().unwrap();

        let commands_with_absolute_paths = config.commands.iter().map(|command| {
            let parent = config_path.parent().unwrap();

            let absolute_audio_file_path = command.audio_file_path.clone().map(|p| {
                parent.join(p).to_string_lossy().to_string()
            });

            let absolute_audio_file_paths = command.audio_file_paths.clone().map(|p| {
                p.iter()
                    .map(|p| parent.join(p).to_string_lossy().to_string())
                    .collect()
            });

            termsfx_config::TermsfxCommandConfig::new(
                command.command.clone(),
                absolute_audio_file_path,
                absolute_audio_file_paths,
            )
        });

        TermsfxConfig {
            commands: commands_with_absolute_paths.collect(),
        }
    }
}
